...
function _find_violations(
    ...
    time_screening = @elapsed begin
        ...
        for sc in instance.buses[1].scenarios
            ...
            overflow_values = [
                value(overflow[sc.name, lm.name, t]) for lm in instance.lines,
                t in 1:instance.time
            ]
            violation_s = UnitCommitmentSTO._find_violations(
                instance = instance,
                sc = sc,
...
`lodf` can be computed using UnitCommitmentSTO.injection_shift_factors and
UnitCommitmentSTO.line_outage_factors. The argument `overflow` specifies how much
