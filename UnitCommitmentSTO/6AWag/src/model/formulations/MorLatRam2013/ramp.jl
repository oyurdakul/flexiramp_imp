...
function _add_ramp_eqs!(
    ...
    for t in 1:model[:instance].time
        ...
        if t == 1
            if is_initially_on
                eq_ramp_up[sn, gn, t] = @constraint(
                    model,
                    g.min_power[t] +
                    prod_above[sn, gn, t] +
                    (RESERVES_WHEN_RAMP_UP ? reserve[sn, gn, t] : 0.0) <=
                    g.initial_power + RU
                )
            end
        else
            ...
            if !time_invariant
                ...
                min_prod_last_period =
                    g.min_power[t-1] * is_on[gn, t-1] + prod_above[sn, gn, t-1]
                eq_ramp_up[sn, gn, t] = @constraint(
                    model,
        ...
        if t == 1
            if is_initially_on
                ...
                eq_ramp_down[sn, gn, t] = @constraint(
                    model,
                    g.initial_power - (g.min_power[t] + prod_above[sn, gn, t]) <= RD
                )
            end
        else
            ...
            if !time_invariant
                ...
                max_prod_last_period =
                    ...
                    prod_above[sn, gn, t-1] +
                    (
                        RESERVES_WHEN_SHUT_DOWN || RESERVES_WHEN_RAMP_DOWN ?
                        reserve[sn, gn, t-1] : 0.0
                    )
                min_prod_this_period =
                    g.min_power[t] * is_on[gn, t] + prod_above[sn, gn, t]
                ...
                eq_ramp_down[sn, gn, t] = @constraint(
                    model,
                    prod_above[sn, gn, t-1] +
                    (RESERVES_WHEN_RAMP_DOWN ? reserve[sn, gn, t-1] : 0.0) -
                    prod_above[sn, gn, t] <= RD
                )
            end
        end
    end
