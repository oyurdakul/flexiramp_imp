    ...
    for t in 1:T
        for k in 1:K
            ...
            if g.min_uptime > 1
                ...
                eq_segprod_limit_a[sn, gn, t, k] = @constraint(
                    model,
                    segprod[sn, gn, t, k] <=
                ...
                eq_segprod_limit_b[sn, gn, t, k] = @constraint(
                    model,
                    segprod[sn, gn, t, k] <=
                    g.cost_segments[k].mw[t] * is_on[gn, t] -
                    Cv * switch_on[gn, t] -
                ...
                eq_segprod_limit_c[sn, gn, t, k] = @constraint(
                    model,
                    segprod[sn, gn, t, k] <=
                    g.cost_segments[k].mw[t] * is_on[gn, t] -
            ...
            eq_prod_above_def[sn, gn, t] = @constraint(
                model,
                prod_above[sn, gn, t] == sum(segprod[sn, gn, t, k] for k in 1:K)
            )
            ...
            add_to_expression!(
                model[:obj],
                tf*s.probability*segprod[sn, gn, t, k],
                g.cost_segments[k].cost[t],
            )

            # Also add an explicit upper bound on segprod to make the solver's
            # work a bit easier
            set_upper_bound(segprod[sn, gn, t, k], g.cost_segments[k].mw[t])
        end
    end
end
