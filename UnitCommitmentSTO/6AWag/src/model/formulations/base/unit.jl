...
function _add_unit!(model::JuMP.Model, sc::Scenario, g::Unit, f::Formulation)
    ...
    # Variables
    _add_production_vars!(model, sc, g, f.prod_vars)
    _add_reserve_vars!(model, sc, g)
    _add_frp_vars!(model, sc, g) # Add variables for flexiramp
    _add_startup_shutdown_vars!(model, g)
    _add_status_vars!(model, g, f.status_vars)

    # Constraints and objective function
    _add_min_uptime_downtime_eqs!(model, g)
    _add_net_injection_eqs!(model,sc, g)
    _add_production_limit_eqs!(model, sc, g)
    ...
    _add_startup_shutdown_limit_eqs!(model, sc, g)
    _add_status_eqs!(model, g, f.status_vars)
    return
end

_is_initially_on(g::Unit)::Float64 = (g.initial_status > 0 ? 1.0 : 0.0)

function _add_production_vars!(model::JuMP.Model, sc::Scenario, g::Unit)::Nothing
...
function _add_production_limit_eqs!(model::JuMP.Model, sc::Scenario, g::Unit)::Nothing
    ...
    for t in 1:model[:instance].time
        ...
        add_to_expression!(model[:obj], tf*sc.probability*is_on[gn, t], g.min_power_cost[t])
        ...
        eq_prod_limit[gn, sc, t] = @constraint(
            model,
            prod_above[sc.name, gn,  t] + reserve[sc.name, gn, t] <= power_diff * is_on[gn, t]
        )
    end
end

function _add_reserve_vars!(model::JuMP.Model, sc::Scenario, g::Unit)::Nothing
    reserve = _init(model, :reserve)
    for t in 1:model[:instance].time
        if g.provides_spinning_reserves[t]
            reserve[sc.name, g.name,  t] = @variable(model, lower_bound = 0)
        else
            reserve[ sc.name, g.name, t] = 0.0
        end
    end
    return
end
...
function _add_startup_shutdown_limit_eqs!(model::JuMP.Model, sc::Scenario, g::Unit)::Nothing
    ...
    for t in 1:T
        ...
        eq_startup_limit[sc.name, g.name, t] = @constraint(
            model,
            prod_above[sc.name, g.name, t] + reserve[sc.name, g.name, t] <=
...
function _add_net_injection_eqs!(model::JuMP.Model, sc::Scenario, g::Unit)::Nothing
