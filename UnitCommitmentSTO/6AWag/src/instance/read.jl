...
"""
    read_benchmark(name::AbstractString)::UnitCommitmentSTOInstance
...
-------
    ...
    instance = UnitCommitmentSTO.read_benchmark("matpower/case3375wp/2017-02-01")
"""
function read_benchmark(name::AbstractString)::UnitCommitmentSTOInstance
    basedir = dirname(@__FILE__)
    return UnitCommitmentSTO.read("$basedir/../../instances/$name.json.gz")
end

"""
    read(path::AbstractString)::UnitCommitmentSTOInstance
...
-------
    ...
    import UnitCommitmentSTO
    instance = UnitCommitmentSTO.read("/path/to/input.json.gz")
"""
function read(path::AbstractString)::UnitCommitmentSTOInstance
    if endswith(path, ".gz")
...
function _read(file::IO)::UnitCommitmentSTOInstance
    return _from_json(
...
function _from_json(json; repair = true)
    ...
    name_to_scenario= Dict{String,Scenario}()
    ...
    frp_shortfall_penalty = timeseries(
    ...
    for (bus_name, dict) in json["Buses"]
        scenarios=Scenario[]
        for (scenario_name, dict2) in dict
            scenario=Scenario(scenario_name, dict2["Load (MW)"], dict2["Probability"])
            name_to_scenario[scenario_name]=scenario
    ...
    if "Reserves" in keys(json)
        ...
        reserves.upfrp =
            timeseries(json["Reserves"]["Up-FRP (MW)"], default = zeros(T))
        reserves.dwfrp =
            timeseries(json["Reserves"]["Down-FRP (MW)"], default = zeros(T))
    end
    ...
    instance = UnitCommitmentSTOInstance(
        ...
        frp_shortfall_penalty = frp_shortfall_penalty,
        time = T,
    ...
    if repair
        UnitCommitmentSTO.repair!(instance)
    end
    return instance
end
